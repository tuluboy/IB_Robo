...
{
	//-------------------Test Code by zhangchao -----------------------//
public:
	
	void GetContractsByExpiration(const std::string& udly, const std::string& exp);
	bool getSecDefOptParams(std::string& underlying, int conid, QryFBSeqDefOptParam& qryfb);
	bool historicalDataRequests(Contract& cont, const char* queryTime, const std::string& durationStr, const std::string&  barSizeSetting, QryFBHistData& qryfb);
	void queryLatestTrades();
	void queryAccountInfo();
	void getOptionPrice(const std::string& StkId, const std::string& expiredate, const double strike, const std::string& OpType);
	void getOptionsByCond0(const std::string& StkId, const long conid, const std::string& expiredate, const double theV);
	long getConId(const std::string& StkId, const std::string& sectype); //sectype:"STK" "OPT"
	void marketDataType();
	void quit();
	bool isquit(){ return ST_QUIT == m_state; };

	double m_curAsk;
	double m_curBid;
	double m_curLast;
	std::vector<OptionChainItem> m_optchain;
	int m_reqid;
	int m_curCmd;
	AccountDisp acctDispItem;
	void setQryFdBk(void* in){ queryFeedBack = in; };
	void resetQryFdBk(){ queryFeedBack = nullptr; };
	//-------------------Test Code by zhangchao -----------------------//
public:
